...
from douentza.views.surveys_stats import (
    export_survey_as_csv,
    compute_survey_questions_data, compute_survey_meta_data_as_questions)
    ...
    def handle(self, *args, **options):
        ...
        def remove_previous_file(cdata):
            if cdata.data_type == CachedData.TYPE_FILE and cdata.value:
                filename = os.path.join(settings.CACHEDDATA_FOLDER,
                                        cdata.value)
                try:
                    os.remove(filename)
                    return True
        ...
        for survey in Survey.validated.all():
            ...
            # Survey Export File
            fname = 'ms{id}_{slug}-{date}.csv'.format(
                id=survey.id, slug=slugify(survey.title),
                date=datestr)
            filename = os.path.join(settings.CACHEDDATA_FOLDER, fname)
            export_survey_as_csv(survey=survey,
                                 filename=filename)

            fcache, _ = CachedData.objects.get_or_create(
                slug=survey.cache_file_slug)
            ...
            # Survey Stats Data
            ocache, _ = CachedData.objects.get_or_create(
                slug=survey.cache_slug)
            ...
            mcache, _ = CachedData.objects.get_or_create(
                slug=survey.meta_cache_slug)
        ...
        gcache, _ = CachedData.objects.get_or_create(
            slug=slugify("general_stats"))
        ...
        jcache, _ = CachedData.objects.get_or_create(
            slug=slugify("general_stats_graph"))
        ...
        jcache, _ = CachedData.objects.get_or_create(
            slug=slugify("geojson_statistics"))
        remove_previous_file(jcache)  # clean-up first
        jcache.data_type = CachedData.TYPE_FILE
        jcache.value = fname
        jcache.created_on = now
